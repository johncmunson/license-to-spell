...
test.describe("Game Initialization", () => {
  ...
  test('"Randomize Plate" or "Start Game" button is visible', async ({
    page,
  }) => {
    await page.goto("/")
    ...
    const startButton = page.getByRole("button", {
      name: /randomize|start|new game/i,
    })
...
test.describe("Starting a Round", () => {
  ...
  test("license plate displays a number between 100 and 999", async ({
    page,
  }) => {
    await page.goto("/")
...
test.describe("Guessing Words", () => {
  test("valid word submission adds word to correct guesses list", async ({
    page,
  }) => {
    await page.goto("/")
  ...
  test("valid word submission increases score by word length", async ({
    page,
  }) => {
    await page.goto("/")
  ...
  test("duplicate guess is rejected", async ({ page }) => {
    ...
    for (const word of commonWords) {
      ...
      if (
        currentGuessesHtml !== initialGuessesHtml &&
        currentGuessesHtml.toLowerCase().includes(word.toUpperCase())
      ) {
        validWord = word
        break
      }
  ...
  test("input clears after valid submission, selected after invalid", async ({
    page,
  }) => {
    await page.goto("/")
...
test.describe("Timer Behavior", () => {
  ...
  test("timer button is clickable during active round (acts as stop)", async ({
    page,
  }) => {
    await page.goto("/")
...
test.describe("Stopping Early", () => {
  test("clicking timer button ends the round", async ({ page }) => {
    ...
    const newRoundButton = page.getByRole("button", {
      name: /new round|play again|restart/i,
    })
